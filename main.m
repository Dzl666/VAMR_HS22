...
if ds == 0
    ...
    img = imread([kitti_path '/05/image_0/' sprintf('%06d.png', bootstrap_frames(2))]);
elseif ds == 1
...
elseif ds == 2
    ...
    img = rgb2gray(imread([parking_path sprintf('/images/img_%05d.png', bootstrap_frames(2))]));
else
    assert(false);
end
...
[R_C2W, t_C2W, kpt_init, P3d_init] = bootstrapKLT(img0, img, camParams, cfgs);
...
for iter = range
    ...
    if size(P_t, 2) < cfgs.min_track_kpts
        ...
        P_t = [P_t, kpt_init];
        % conver the rebooted P3d from prev_prev_img coord. to world coord. R_CW * P + (-R_WC' * t_WC)
        ...
        ViewId = uint32(1);
        t_C2W_prev = -poseW2C_hist{1, iter-1}' * poseW2C_hist{2, iter-1};
        AbsolutePose = rigidtform3d(poseW2C_hist{1, iter-1}', t_C2W_prev');
    ...
    [P_t, X_t, C_t, F_t, Tao_t, C_cnt_t, T_W2C] = continuousTracking(prev_img, img, ...
        camParams, cfgs, P_t, X_t, C_t, F_t, Tao_t, C_cnt_t);
...
%     h = plot3(X_prev(1,:), X_prev(2,:), X_prev(3,:), 'ko');
    ...
    xlim([traj(1,1)-20, traj(1,1)+20]);
    ylim([traj(1,2)-20, traj(1,2)+20]);
    zlim([traj(1,3)-20, traj(1,3)+20]);
...
if cfgs.plot_final_path
    figure(4)
    traj = zeros(iter, 3);
    for k = [1,2, bootstrap_frames(end):iter]
        traj(k, :) = -poseW2C_hist{1,k}' * poseW2C_hist{2,k};
    end
    plot3(traj(:, 1), traj(:, 2), traj(:, 3), 'r-'); hold on;
    plot3(traj(:, 1), traj(:, 2), traj(:, 3), 's');
    if ds == 0
        xlim([-100,200])
        zlim([-60,200])
    end
    view(0,0); axis equal; grid on;
